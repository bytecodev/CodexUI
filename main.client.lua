...
do
	...
	if ok then
		...
		if cloneref(game:GetService("RunService")):IsStudio() then
			CodexUI = require(cloneref(ReplicatedStorage:WaitForChild("CodexUI"):WaitForChild("Init")))
		else
			CodexUI =
				loadstring(game:HttpGet(
				"https://raw.githubusercontent.com/bytecodev/CodexUI/main/dist/main.lua" -- custom fork; includes Tab:EmptyState()
			))()
		end
	end
end
...
CodexUI.Creator.AddIcons("solar", {
...
function createPopup()
	return CodexUI:Popup({
		Title = "Welcome to the CodexUI!",
		Icon = "bird",
		Content = "Hello!",
		Buttons = {
			{
...
-- */  Window  /* --
local Window = CodexUI:CreateWindow({
	Title = "CodexAlpha  |  CodexUI Example",
	--Author = "by CodexAlpha",
	Folder = "codexui-example",
	...
	OpenButton = {
		Title = "Open CodexUI", -- can be changed
...
do
	...
	AboutTab:Button({
		Title = "Export CodexUI JSON (copy)",
		Color = Color3.fromHex("#a2ff30"),
		Justify = "Center",
		IconAlign = "Left",
		Icon = "", -- removing icon
		Callback = function()
			tableToClipboard(CodexUI)
			CodexUI:Notify({
				Title = "CodexUI JSON",
...
do
	...
	assert(type(EmptyStateTab.EmptyState) == "function", "CodexUI build is outdated: Tab:EmptyState() is missing. Update this fork's dist/main.lua.")
	...
	-- EmptyState supports normal CodexUI elements such as Button/Paragraph/Input.
	EmptyState:Button({
		Title = "Connect first",
...
do
	...
	ButtonTab:Button({
		...
		Callback = function()
			CodexUI:Notify({
				Title = "Hello",
				Content = "Welcome to the CodexUI Example!",
...
if not RunService:IsStudio() and writefile and printidentity() then
	do -- config elements
		...
		-- All elements are taken from the official documentation: CodexUI README / source comments
...
-- */  Other  /* --
do
	local InviteCode = "KxYmkDVEg3"
	local DiscordAPI = "https://discord.com/api/v10/invites/" .. InviteCode .. "?with_counts=true&with_expiration=true"

	local Response = CodexUI.cloneref(game:GetService("HttpService"))
		:JSONDecode(CodexUI.Creator.Request and CodexUI.Creator.Request({
			Url = DiscordAPI,
			Method = "GET",
			Headers = {
				["User-Agent"] = "CodexUI/Example",
